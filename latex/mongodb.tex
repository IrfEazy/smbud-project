...
\begin{lstlisting}[label={lst:lstlisting51}]
{
    "_id": <String>,
    "title": <String>,
    "authors": [ {
...
\end{lstlisting}
Below we provide some examples of the three main papers types that appear in our document with their associated attributes.
...
\paragraph{Model of the journal:}
This is the document structure for the papers published in journals.
\begin{figure}[H]
    \begin{center}
        \includegraphics[width=0.6\linewidth]{ImagesMongoDB/journal}
        \label{fig:journal}%
    \end{center}
\end{figure}

\paragraph{Model of the book:}
...
\paragraph{Model of the conference:}
...
One drawback is the duplication of data, for example it is possible that an author has written more than one paper that is stored in the database, so the author's information are saved multiple times.
...
\footnotemark\footnotetext{Amazon Fine Food Reviews, accessed on the 14/11/2022
...
\begin{enumerate}
    ...
    \footnotemark\footnotetext{mongod is the primary daemon process for the MongoDB system}
...
db.bibliography.insertOne( {
    ...
    \begin{lstlisting}[label={lst:lstlisting56}]
db.bibliography.aggregate( [ {
    ...
    The command \verb|updateOne| allows to search for a document that matches a specific filter and modify it accordingly to the update rules expressed inside the query.
...
db.bibliography.deleteMany( { date: { $lt: ISODate("1950-01-01T00:00:00.000Z") } } )
    ...
    \begin{figure}[H]
        \begin{center}
            \includegraphics[width=0.6\textwidth]{ImagesMongoDB/delete}
            \label{fig:delete}%
        \end{center}
    \end{figure}
    ...
    \begin{lstlisting}[label={lst:lstlisting61}]
db.bibliography.updateMany( {
...
\begin{figure}[H]
    \begin{center}
        \includegraphics[width=0.3\textwidth]{ImagesMongoDB/q5mongo}
        \label{fig:q5mongo}%
    \end{center}
\end{figure}
...
db.bibliography.find( {
    ...
    \end{itemize}
    We can clearly see that the the documents passing the filter are about 4\% of the total, so we can say that this query is selective enough and can proceed to test it with the addition of an index.
    ...
    We proceed creating an index on the field \verb|date| and we run the query again.
    This time the execution plan and stats return different values:
    \begin{figure}[H]
        \begin{center}
            \includegraphics[width=0.3\linewidth]{ImagesMongoDB/ex_stat_indx}
            \label{fig:ex_stat_indx}%
        \end{center}
    \end{figure}
    ...
    In this particular case we also have to access the DB (as shown by the \verb|totalDcosExamined| parameter) because the \verb|publication_type| field is not stored in a index, so we have to retrieve the value of this field from the document itself to complete the filtering condition
    \footnotemark\footnotetext{This type of query, where not all the field needed to compute the query are stored as indexes are called "not covered queries"}.
    ...
    \begin{lstlisting}[label={lst:lstlisting63}]
db.bibliography.createIndex( {
    title: "text" } )
    \end{lstlisting}
    \begin{figure}[H]
        \begin{center}
            \includegraphics[width=0.6\linewidth]{ImagesMongoDB/query7mongo1}
            \label{fig:query7mongo1}%
        \end{center}
    \end{figure}
    ...
    \begin{lstlisting}[label={lst:lstlisting64}]
db.bibliography.find( {
    ...
    \begin{figure}[H]
        \begin{center}
            \includegraphics[width=0.3\linewidth]{ImagesMongoDB/query7mongo2}
            \label{fig:query7mongo2}%
        \end{center}
    \end{figure}
    \item \textbf{Visualize the common values between field of study and keywords in each document}\\
...
db.bibliography.aggregate( [ {
    ...
    \begin{figure}[H]
        \begin{center}
            \includegraphics[width=0.6\linewidth]{ImagesMongoDB/performanceV2}
            \label{fig:performanceV2}%
        \end{center}
    \end{figure}
    ...
    \begin{lstlisting}[label={lst:lstlisting68}]
db.bibliography.aggregate( [ {
    $match: { $and: [ {
    ...
    \begin{figure}[H]
        \begin{center}
            \includegraphics[width=0.6\linewidth]{ImagesMongoDB/query9mongo}
            \label{fig:query9mongo}%
        \end{center}
    \end{figure}
    ...
    \begin{lstlisting}[label={lst:lstlisting69}]
db.bibliography.aggregate( [ {
    $match: { $and: [ {
        sections: { $size: 4 } }, {
        sections: { $elemMatch: {
    ...
    \begin{figure}[H]
        \begin{center}
            \includegraphics[width=0.6\textwidth]{ImagesMongoDB/q10mongo}
            \label{fig:q10mongo}%
        \end{center}
    \end{figure}
    ...
    The output list is ordered in descending order, showing which are the organizations whose researchers have published the most.
    Notice that a paper published by authors from different organizations is counted for all the organizations involved.
    In the first part of the query we unwind the documents by \verb|authors| and do a filtering of inconsistent data, since the focus of this query is to know which are the organizations that have published the most.
    ...
    If we hadn't done this, the same paper would have been counted multiple times, for all the author who participated in writing it and are part of the same organization, inflating that organization's contribution.
    ...
    \begin{lstlisting}[label={lst:lstlisting70}]
db.bibliography.aggregate( [ {
    $unwind: "$authors" }, {
    $match: { $and: [ {
        "authors.org": { $ne: null } }, {
    ...
    \begin{figure}[H]
        \begin{center}
            \includegraphics[width=0.6\textwidth]{ImagesMongoDB/q11.1mongo}
            \label{fig:q11_1mongo}%
        \end{center}
    \end{figure}
    \item \textbf{Number of pages published on Journals by authors affiliated with a certain organization }\\
    ...
    This operation achieve the same goal as the one described in the query before.
    ...
    Finally, the last stages are only for projection and pretty printing the output.
    \begin{lstlisting}[label={lst:lstlisting71}]
db.bibliography.aggregate( [ {
    $unwind: "$authors" }, {
    $match: { $and: [ {
        "authors.org": { $ne: null } }, {
    ...
    \begin{figure}[H]
        \begin{center}
            \includegraphics[width=0.9\textwidth]{ImagesMongoDB/q11mongo}
            \label{fig:q11mongo}%
        \end{center}
    \end{figure}
    \item \textbf{Couples of field of study and keyword which appear more frequently within the papers}\\
    ...
    An initial filtering is done in order to eliminate the papers which doesn't contain any keyword or any field of study.
    The two clauses for doing the filtering can be modified increasing the required number of keywords or the required number of fields of study.
    This kind of change can be interesting in order to understand which fields of study and keywords become more relevant when the subset of considered paper is different.
    ...
    We filter out the couples of fields of study and keywords with less then 100 in order to get rid of some misleading information due to not coupled elements.
    The threshold can be adjusted.
    \begin{lstlisting}[label={lst:lstlisting72}]
db.bibliography.aggregate( [ {
    $match: {
        $expr: { $gte: [ {
            $size: "$fos" },
    ...
    \begin{figure}[H]
        \begin{center}
            \includegraphics[width=0.6\linewidth]{ImagesMongoDB/query12mongo}
            \label{fig:query12mongo}%
        \end{center}
    \end{figure}
    \item \textbf{Papers that reference old papers with same publisher}\\
...
db.bibliography.aggregate( [ {
    ...
    \begin{figure}[H]
        \begin{center}
            \includegraphics[width=0.9\textwidth]{ImagesMongoDB/q15mongo}
            \label{fig:q15mongo}%
        \end{center}
    \end{figure}
    ...
    \begin{lstlisting}[label={lst:lstlisting74}]
db.bibliography.aggregate( [ {
    ...
    \begin{figure}[H]
        \begin{center}
            \includegraphics[width=0.6\linewidth]{ImagesMongoDB/query16mongo}
            \label{fig:query16mongo}
        \end{center}
    \end{figure}
    ...
    An author does an auto-reference when the considered paper presents a reference to another paper the author has wrote.
